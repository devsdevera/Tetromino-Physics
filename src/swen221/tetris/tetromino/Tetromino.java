...
public abstract class Tetromino {
  ...
  public final void rotateLeft() {
    //moveDown();//TODO: fix this code	// replaced moveDown with rotateRight three times
...
//			 	!Board.xOk(x(i)) || 
//				!Board.yOk(y(i)) || 
//				!Board.rangeX().anyMatch(j -> j.equals(x(i))) ||
//				!b.read(x(i), y(i)).map(c -> c.s.equals("_")).orElse(false))
//			 .findFirst().isPresent(); 
	  
	  // ABOVE IS A USEFUL PREVIOUS ITERATION THAT WORKED WHICH LED TO SIMPLIFIED EXPRESSION BELOW
	 
	 return Board.rangeT() 	// stream 0,1,2,3
			 .anyMatch(i ->
			 	!b.read(x(i), y(i)).map(c -> c.s.equals("_")).orElse(false));
	 
	 // invalid x or y coordinates will return Optional<Empty> anyway which is caught and returns true
	 // if the square, x and y, is NOT white (aka already colored) returns true (overlap)
  }
  ...
  public void copyOnBoard(Board b) {//TODO: fix this code
    Board.rangeT()
      .forEach(i->b.write(x(i),y(i),color()));    // changed from 0 to x(i)
  }
}