...
public class Board {
  ...
  private boolean isRowFull(int y) {
     ...
     // optional<> can be followed by a .map to turn it into a boolean (in this case)
     
     return rangeX()
    		 .allMatch(i ->	// return true if all x in y row are not empty (allMatch)
    		 	read(i, y).map(c -> !c.s.equals("_")).orElse(false));
  ...
  public void clearFullRows() {
    ...
    //TODO: complete this code
	  ...
	  reverseRangeY().filter(i ->		// reverseRangeY passes a Stream
	  	isRowFull(i))					// filter asks for a boolean if row is Full
	  	.forEach(j -> clearRow(j));		// For each of the full rows, do a task, clearRow
  }
